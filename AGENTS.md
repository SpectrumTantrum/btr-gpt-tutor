...
- **Classroom** (`lib/classroom/`) — AI-led lesson generation from KB documents: generation pipeline (outline → scenes), playback engine, action engine, multi-agent orchestration (director), and discussion system
...
Tables (Dexie v4): `knowledgeBases`, `documents`, `chunks`, `sessions`, `memory`, `settings`, `notebooks`, `notebookRecords`, `quizzes`, `quizAttempts`, `guides`, `classrooms`
...
- `classroom-store` — classroom generation + playback state
...
- `app/api/` — REST endpoints:
  ...
  - Activities: `quiz/generate`, `quiz/grade`, `guide/plan`, `guide/page`, `solve`, `research`, `classroom/generate`, `classroom/[id]`, `classroom/chat`
...
All domain types (~40 exports covering knowledge, session, memory, quiz, guide, notebook, classroom, plugin, search) live in `lib/core/types/index.ts` — it is the single source of truth, read it first when touching types. All properties are `readonly`.
